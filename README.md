...
**ASROCK** Десктопные платы: Идут после Асуса по качеству и взоможностям. Питальник похуже чем у ASUS но всё равно всё хорошо. Брать тоже можно. Серверные платы: Ничего не знаю кроме того что есть платы с DDR3 памятью. Ещё у ASROCK есть mITX платы под 2011-3, единственные под этот сокет. Так же у некоторых плат присутсвует аналог Асусовского OC Socket, например на: ASRock X99 OC Formula/3.1, и других.
...
**MSI:** Имеют проблемы с запуском инженерников v3 и памятью которая выше 2133. *Так же не поддерживают E5 4600 V3.* Возможно только ES и QS.
 ...
 *Примечание:* Если вы просматриваете эту страницу с браузера на основе Chromium: Google Chrome, Yandex, Opera, Vivaldi, MS Edge Beta 2019, etc. то фото материнкой платы вы не увидите. Но вы его можете просмотреть [тут](https://github.com/ThinkPadThink/Guide2011-3/blob/master/X99_AD3.jpg?raw=true)
...
Примечание: *косячный ASUSовский UEFI/BIOS+Windows 10 из-за чего все ядра работают всегда на частоте 2.8Ghz и в однопотоке CPU не может выйти в турбобуст (3.8) на 1 ядро*
