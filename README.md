...
В любом случае понадобится переделка стандартного разъёма с блока питания, либо напрямую либо через переходник-удлинитель. 
 ...
 А также разъём для удлинителя и провода, ну и паяльник, припой, флюс и мультиметр если у вас этого нет.
...
Далее собираем по данной фотографии переходник:
 ...
 После этого запускаем блок питания без подключения платы, и проверяем напряжение на всех разъёмах. И при необходимости подкручиваем на приобразователе для получения 12в на дежурке.
